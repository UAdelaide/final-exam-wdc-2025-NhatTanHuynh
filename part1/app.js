...
app.get('/api/walkers/summary', async (req, res) => {
    try {
        const query = 'SELECT u.user_id walker_username, COUNT (r.reating) AS total_ratings, AVG(r.rating) AS average_rating,COUNT (CASE WHEN wr.status = "accepted" AND wr.status = "completed" THEN 1 END) AS completed_walks FROM Users u LEFT JOIN WalkApplicatios wa ON u.user_id = wa.walker';
    } catch (error) {
